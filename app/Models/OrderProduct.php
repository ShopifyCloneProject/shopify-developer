...
class OrderProduct extends Model
{
    ...
    {
        $imageSrcData = [];
        $objProductMedium = ProductMedium::where('src',$this->attributes['src'])->first();
        if(!empty($objProductMedium)){
            $imageSrcData = $objProductMedium->image_src;
        }
        else
        {
            $imageSrcData = ["/assets/images/no-image.jpg","/assets/images/no-image.jpg","/assets/images/no-image.jpg","/assets/images/no-image.jpg"];            
        }
        return $imageSrcData;
    }
    ...
    public function getCreatedAtAttribute($value)
    {
        ...
        $objUserTimeZone = UserStore::where('user_id',config('client_id'))->latest()->first();
    ...
    public function getDeletedAtAttribute($value)
    {
        ...
        $objUserTimeZone = UserStore::where('user_id',config('client_id'))->latest()->first();
        if(!empty($objUserTimeZone) && !empty($value))
        {
            $objUserSelectedTimeZone = $objUserTimeZone->time_zone->timezone_value;
            $time = Carbon::parse($value, 'UTC')->setTimezone($objUserSelectedTimeZone);
            $value = $time->format('Y-m-d H:i:s');

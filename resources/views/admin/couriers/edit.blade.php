...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.couriers.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.couriers.update", [$couriers->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.couriers.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $couriers->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('courier_code') ? 'is-invalid' : '' }}" type="text" name="courier_code" id="courier_code" value="{{ old('courier_code', $couriers->courier_code) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Courier::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $couriers->status) === (string) $key ? 'checked' : '' }} required>

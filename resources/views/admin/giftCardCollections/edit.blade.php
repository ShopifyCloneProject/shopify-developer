...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.giftCardCollection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gift-card-collections.update", [$giftCardCollection->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('gift_card') ? 'is-invalid' : '' }}" name="gift_card_id" id="gift_card_id">
                    @foreach($gift_cards as $id => $entry)
                        <option value="{{ $id }}" {{ (old('gift_card_id') ? old('gift_card_id') : $giftCardCollection->gift_card->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('collection') ? 'is-invalid' : '' }}" name="collection_id" id="collection_id">
                    @foreach($collections as $id => $entry)
                        <option value="{{ $id }}" {{ (old('collection_id') ? old('collection_id') : $giftCardCollection->collection->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

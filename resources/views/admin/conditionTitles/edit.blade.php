...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.conditionTitle.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.condition-titles.update", [$conditionTitle->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.conditionTitle.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $conditionTitle->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ConditionTitle::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $conditionTitle->status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('collection_condition') ? 'is-invalid' : '' }}" type="text" name="collection_condition" id="collection_condition" value="{{ old('collection_condition', $conditionTitle->collection_condition) }}" required>

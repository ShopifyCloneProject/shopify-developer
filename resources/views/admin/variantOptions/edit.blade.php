...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.variantOption.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.variant-options.update", [$variantOption->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="options">{{ trans('cruds.variantOption.fields.options') }}</label>
                <input class="form-control {{ $errors->has('options') ? 'is-invalid' : '' }}" type="text" name="options" id="options" value="{{ old('options', $variantOption->options) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\VariantOption::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $variantOption->status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('variant') ? 'is-invalid' : '' }}" name="variant_id" id="variant_id" required>
                    @foreach($variants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('variant_id') ? old('variant_id') : $variantOption->variant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

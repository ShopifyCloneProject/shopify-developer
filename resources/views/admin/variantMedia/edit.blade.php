...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.variantMedium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.variant-media.update", [$variantMedium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_variant') ? 'is-invalid' : '' }}" name="product_variant_id" id="product_variant_id">
                    @foreach($product_variants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_variant_id') ? old('product_variant_id') : $variantMedium->product_variant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $variantMedium->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('src') ? 'is-invalid' : '' }}" type="text" name="src" id="src" value="{{ old('src', $variantMedium->src) }}">
                @if($errors->has('src'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('src_alt_text') ? 'is-invalid' : '' }}" type="text" name="src_alt_text" id="src_alt_text" value="{{ old('src_alt_text', $variantMedium->src_alt_text) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_default') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_default" id="is_default" value="1" {{ $variantMedium->is_default || old('is_default', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reorder') ? 'is-invalid' : '' }}" type="number" name="reorder" id="reorder" value="{{ old('reorder', $variantMedium->reorder) }}" step="1">

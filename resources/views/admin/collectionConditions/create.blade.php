...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.collectionCondition.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.collection-conditions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="model_name">{{ trans('cruds.collectionCondition.fields.model_name') }}</label>
                <input class="form-control {{ $errors->has('model_name') ? 'is-invalid' : '' }}" type="text" name="model_name" id="model_name" value="{{ old('model_name', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('model_ref') ? 'is-invalid' : '' }}" type="text" name="model_ref" id="model_ref" value="{{ old('model_ref', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('collection_title') ? 'is-invalid' : '' }}" name="collection_title_id" id="collection_title_id" required>
                    @foreach($collection_titles as $id => $entry)
                        <option value="{{ $id }}" {{ old('collection_title_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('condition') ? 'is-invalid' : '' }}" name="condition_id" id="condition_id" required>
                    @foreach($conditions as $id => $entry)
                        <option value="{{ $id }}" {{ old('condition_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.userStore.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.settings.user-stores.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="store_contact_email">{{ trans('cruds.userStore.fields.store_contact_email') }}</label>
                <input class="form-control {{ $errors->has('store_contact_email') ? 'is-invalid' : '' }}" type="email" name="store_contact_email" id="store_contact_email" value="{{ old('store_contact_email') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sender_email') ? 'is-invalid' : '' }}" type="text" name="sender_email" id="sender_email" value="{{ old('sender_email', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('unit_system') ? 'is-invalid' : '' }}" name="unit_system" id="unit_system" required>
                    ...
                    @foreach(App\Models\UserStore::UNIT_SYSTEM_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('unit_system', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('unit_weight') ? 'is-invalid' : '' }}" name="unit_weight" id="unit_weight" required>
                    ...
                    @foreach(App\Models\UserStore::UNIT_WEIGHT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('unit_weight', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>

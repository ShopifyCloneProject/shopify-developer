      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">Best trends section</h4>
                ...
                <div class="col-sm-12">
                  <div class="demo-inline-spacing layout-sec mb-2">
                      ...
                      <div class="custom-control custom-control-primary custom-checkbox">
                          <input type="checkbox" class="custom-control-input" id="besttrends" v-model="list.status"  />
                          <label class="custom-control-label" for="besttrends"></label>
                      </div>
                  </div>
            ...
            <div class="row mt-1">
              ...
              <div class="col-sm-10" id="slidersection">
                    <section>
                        ...
                        <template>
                        <form ref="trendsmediaform">
                        <input type="file" name="trendsmedia" accept="image/jpeg, image/png" id="trendsmedia" class="opacity-0" @change="passSliderImagedata($event)">
                        </form>
                        </template>
              </div>
            </div>
             ...
             <div class="row mt-2" v-if="trendsData.length > 0">
                <div class="col-12">
                  <div class="responsive-div-table table">
                        <div class="row header">
                          <div class="cell w-15">Move</div>
                  ...
                  <draggable class="responsive-div-table" v-model="trendsData" :disabled="!dragdisabled" @end="endDrag"  draggable=".menuitem">
                    <div class="menuitem row w-100 pl-1" v-for="(trendsdata, index) in trendsData" >
                      <div class="cell w-15  p-1"><a href="javascript:void(0)" @mouseover="setDraggableMenu(true)" @mouseleave="setDraggableMenu(false)"><i data-feather='move'></i></a> </div>
                      <div class="cell w-20  p-1">{{ trendsdata.text1 }}</div>
                      <div class="cell w-20  p-1">{{ trendsdata.text2 }}</div>
                      <div class="cell w-25  p-1">{{ trendsdata.url }}</div>
                      <div class="cell w-10  p-1">  
                        <img  class="main-image" :src="trendsdata.imagedata[0].imageurl"  width="100%" height="50px"  alt=""  @error="setAltImg">
                      </div>
...
import draggable from 'vuedraggable'
  ...
  export default {
    ...
    mounted() {
        this.trendsData = this.list.pagemedias;
    ...
    methods: {
      tempSaveData(){
        ...
        this.trendsData.push(this.formData);
        this.resetData();
      ...
      setWithParent(){
        this.list.pagemedias = this.trendsData;
      },
      removeMenu(index)
      {
        this.trendsData.splice(index,1);
        this.setWithParent();
      },
       callFileData(){
            $("#trendsmedia").trigger('click');
        },
        passSliderImagedata(event){
            ...
            self.$refs.trendsmediaform.reset();

      ...
      <div class="holder breadcrumbs-wrap mt-0">
         <div class="container">
            <ul class="breadcrumbs">
               ...
               <li><span> {{ lang.global.collection.products }} </span></li>
            </ul>
         </div>
   </div>
   <div class="holder">
      <div class="container collection-page-content" id="detailSection">
         <!-- Two columns -->
         <!-- Page Title -->
         <div class="page-title text-center">
            <h1> {{ lang.global.shop.shop }} </h1>
         </div>
         <!-- /Page Title -->
         <!-- Filter Row -->
         <div class="filter-row" v-if="products.length > 0">
            <div class="row">
               ...
               <div class="select-wrap d-none d-md-flex">
                  ...
                  <div class="select-wrapper select-wrapper-xxs" @change="getProducts('filter')">
               ...
               <div class="select-wrap d-none d-md-flex">
                  ...
                  <div class="select-wrapper select-wrapper-xxs">
                     <select class="form-control input-sm" v-model="perPage" @change="getProducts('filter')">
         ...
         <div class="row">
            ...
            <div class="col-lg-4 aside aside--left filter-col filter-mobile-col filter-col--sticky js-filter-col filter-col--opened-desktop" data-grid-tab-content>
               <div class="filter-col-content filter-mobile-content">
                     ...
                     <div class="sidebar-block_content">
                        <div class="selected-filters-wrap">
                           <ul class="selected-filters">
                              <template v-if="filter.collections.length > 0">
                                 <li v-for="(collection,index) in data.collections">
                                    <a class="pointer" @click="removeFilter('size', collection.id)" v-if="filter.collections.includes(collection.id)">{{collection.title}}</a>
                                 </li>
                              </template>
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenCollection ? 'open' : 'collapsed'" v-if="data.collections.length > 0">
                     <div class="sidebar-block_title" @click="isOpenCollection = !isOpenCollection">
                        <span> {{ lang.global.shop.category }} </span>
                     ...
                     <div class="sidebar-block_content">
                        <ul class="tags-list">
                           <li v-for="(collection, index) in data.collections">
                              <div class="clearfix">
                                 <input :id="'collection_'+index" :value="collection.id" type="checkbox" v-model="filter.collections" @change="getProducts('filter')">
                                 <label :for="'collection_'+index">{{collection.title}}</label>
                              </div>
                           </li>
                        </ul>
                     </div>
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenSize ? 'open' : 'collapsed'" v-if="data.size.length > 0">
                     ...
                     <div class="sidebar-block_content">
                        <ul class="category-list two-column size-list">
                           <li v-for="(size, index) in data.size">                              
                              <div class="clearfix">
                                 <input :id="'size'+index" :value="size.id" type="checkbox" v-model="filter.size" @change="getProducts('filter')">
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenColor ? 'open' : 'collapsed'" v-if="data.color.length > 0">
                     ...
                     <div class="sidebar-block_content">
                        <ul class="color-list two-column">
                           <li v-for="(color, index) in data.color">
                              <div class="clearfix">
                                 <input :id="'color'+index" :value="color.id" type="checkbox" v-model="filter.color" @change="getProducts('filter')">
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenMaterial ? 'open' : 'collapsed'" v-if="data.material.length > 0">
                     ...
                     <div class="sidebar-block_content">
                        <ul class="category-list">
                            <li v-for="(material, index) in data.material">                              
                              <div class="clearfix">
                                 <input :id="'material'+index" :value="material.id" type="checkbox" v-model="filter.materials" @change="getProducts('filter')">
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenStyle ? 'open' : 'collapsed'" v-if="data.style.length > 0">
                     ...
                     <div class="sidebar-block_content">
                        <ul class="category-list">
                            <li v-for="(style, index) in data.style">                              
                              <div class="clearfix">
                                 <input :id="'style'+index" :value="style.id" type="checkbox" v-model="filter.style" @change="getProducts('filter')">
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenTitle ? 'open' : 'collapsed'" v-if="data.title.length > 0">
                     ...
                     <div class="sidebar-block_content">
                        <ul class="category-list">
                            <li v-for="(title, index) in data.title">                              
                              <div class="clearfix">
                                 <input :id="'title'+index" :value="title.id" type="checkbox" v-model="filter.titles" @change="getProducts('filter')">
                     ...
                     <div class="sidebar-block_content">
                        <ul class="category-list">
                            <li v-for="(vendor, index) in data.vendors">                              
                              <div class="clearfix">
                                 <input :id="'vendor'+index" :value="vendor.id" type="checkbox" v-model="filter.brand" @change="getProducts('filter')">
                  ...
                  <div class="sidebar-block filter-group-block" :class="isOpenPrice ? 'open' : 'collapsed'">
                     ...
                     <div class="sidebar-block_content">
                        ...
                        <div class="text-center mt-1">
                           <button type="button" class="btn btn-primary" @click="getProducts('filter')"> {{ lang.global.search }} </button>
                        </div>
                     </div>
                  </div>
                     ...
                     <div class="sidebar-block_content populer-tags" id="scrollbar-style">
                        <ul class="tags-list">
                           <li v-for="(tag, index) in data.tags">                              
                              <div class="clearfix">
                                 <input :id="'tag'+index" :value="tag.id" type="checkbox" v-model="filter.tags" @change="getProducts('filter')">
            ...
            <div class="col-lg aside">
               <div class="prd-grid-wrap">
                 ...
                 <!--  <div class="section-pagination d-none" v-if="!isProductFound">
                     <sliding-pagination
                       :current="currentPage"
                       :total="totalPages"
...
export default {
    ...
    computed: {
      ...
      getTotalSelection(){
         return parseInt(this.filter.collections.length) + parseInt(this.filter.size.length) + parseInt(this.filter.color.length) + parseInt(this.filter.brand.length) + parseInt(this.filter.tags.length) + parseInt(this.filter.material.length) + parseInt(this.filter.style.length) + parseInt(this.filter.titles.length);
      },
    },
    created(){
    ...
    methods: {
      ...
      getProducts(type = null){
         if(!this.queriedProductAPI){
            ...
            var payload = {pid: this.currentPage, sortType:this.sortType, perPage:this.perPage, price:this.filter.price};
            if(this.filter.collections.length > 0){ payload.collections = this.filter.collections; }
            ...
            this.$store.dispatch("globalStore/getProducts", payload)
            .then((res) => {

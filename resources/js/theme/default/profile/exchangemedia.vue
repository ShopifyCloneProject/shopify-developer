        ...
        <div class="card" id="mediaDataId">
            ...
            <div class="card-body">
                <section>
                    ...
                    <div class="draggable-outer-line" v-if="exchangeMediaData.length == 0"> 
                    ...
                    <div class="DropZone__Container hasitem" v-else>
                        <div class="dropzone-first">
                            <draggable tag="div" v-model="exchangeMediaData" class="dropzone-second-div" draggable=".item">
                                <div v-for="(imagedata,index) in exchangeMediaData" class="item">
...
export default {
    ...
    mounted(){
        let self = this;
        setTimeout(function(){
            self.exchangeMediaData = self.exchangemedia; 
        }, 500);
    },
    methods:{
        callFileData(){
        ...
        mouseOverDiv(index){
            this.exchangeMediaData[index].displaycheckbox = true;
        },
        mouseLeaveDiv(index){
            this.exchangeMediaData[index].displaycheckbox = false;
            this.checkAnySelect();
        },
        appendMedia(base64image){
            this.exchangeMediaData.push({id: Math.floor((Math.random() * -10000000)),  imageurl: base64image, checked: false, displaycheckbox: false, media: [] });
        },
        checkAnySelect(){
            let funCheckDisplayCheckbox = false;
            for (const [key, value] of Object.entries(this.exchangeMediaData)) {
                if(value.checked)
                {
        ...
        deleteMediaIndex(){
            for (const [key, value] of Object.entries(this.exchangeMediaData)) {
                if(value.checked)
                {
                    this.exchangeMediaData.splice(key, 1);
                    this.deleteMedia();
                }
            }
        },

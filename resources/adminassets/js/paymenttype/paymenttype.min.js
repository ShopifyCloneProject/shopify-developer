    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            initComplete: function(){
                ...
                if($("#payment_type_create").length != 1)
                {
                    $("#btnCreate").remove();
                }

                if($("#payment_type_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#payment_type_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#payment_type_export_access").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Payment Type Listing</h6>');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(res.status_code == 2058){
                    ...
                    unblockSection(section);
                    errorModal(err.response.message, 'Error!');
                }
            },
            error: function(data) {
                unblockSection(section);
        ...
        Swal.fire({
            ...
            if (result.value) {
              deleteRecord(id);
               ...
               successModal('Your payment type is safe :)', 'Cancelled');
            }
        });
    });

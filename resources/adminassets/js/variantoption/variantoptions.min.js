$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_basic = dt_filter_table.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        return (
                            '<a href="javascript:;" class="item-edit btn_edit" data-id="'+full.id+'" >' +
                            feather.icons['edit'].toSvg({
                                class: 'font-small-4'
                            }) +
                            '</a>'+
                            ' <a href="javascript:;" class="m-l-5 delete-record btn_delete" data-id="'+full.id+'">' +
            ...
            buttons: [
                {
                    extend: 'collection',
                    className: 'btn btn-outline-secondary btn-export dropdown-toggle mr-2',
                    attr:{
                        'id':'btn_variantoption_export'
                    },
            ...
            initComplete: function(){
                if($('#variant_option_create').length != 1)
                {
                    $('#create_new').remove();
                }
                if($('#variant_option_edit').length != 1)
                {
                    $('.btn_edit').remove();
                }
                if($('#variant_option_delete').length != 1)
                {
                    $('.btn_delete').remove();
                }
                if($('#variant_option_export').length != 1)
                {
                    $('#btn_variantoption_export').remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Variant Types Listing</h6>');
    ...
    $('.data-submit').on('click', function(e) {
        e.preventDefault();
        var formData = {
            options: jQuery('#addOption').val(),
            variant_id: jQuery('#addVariant').val(),
    ...
    $(document).on('click', '.item-edit', function(){
        ...
        $('#optionsId').val(data.id);
        $('#editOption').val(data.options);
        let status = data.status == 'Active' ? 1 : 0;
        $('#editStatus').val(status);
        $('#editVariant').val(data.variant_id);
        $('#default').modal('show');
    });
    ...
    $(document).on('click', '.data-edit', function(){
        ...
        let id = $('#optionsId').val(),
        options = $('#editOption').val(),
        variant_id = $('#editVariant').val(),

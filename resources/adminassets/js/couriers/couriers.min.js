$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            initComplete: function(){
                ...
                if($("#couriers_create").length != 1)
                {
                    $("#btnCreate").remove();
                }

                if($("#couriers_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#couriers_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#couriers_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Couriers Listing</h6>');
    ...
    $('.data-submit').on('click', function(e) {
        ...
        e.preventDefault();
        var formData = {
            name: jQuery('#addName').val(),
            courier_code: jQuery('#addCourierCode').val(),
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('couriers')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('couriers')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected couriers');
            }
        });
    });
    ...
    $(document).on('click', '.item-edit', function(){
        ...
        $('#couriersId').val(data.id);
        $('#editName').val(data.name);
        $('#editCourierCode').val(data.courier_code);
        let status = data.status == 'Active' ? 1 : 0;
        $('#editStatus').val(status);
        $('#editShippingMethodId').val(data.shipping_method_id);
        $('#default').modal('show');
    });
    ...
    $(document).on('click', '.data-edit', function(){
        openLoader();
        let id = $('#couriersId').val(),
        name = $('#editName').val(),
        courier_code = $('#editCourierCode').val(),
        status = $('#editStatus').val(),
        shipping_method_id = $('#editShippingMethodId').val();

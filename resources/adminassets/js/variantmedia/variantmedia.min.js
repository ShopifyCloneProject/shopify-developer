$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            initComplete: function(){
                ...
                if($("#variant_medium_create").length != 1)
                {
                    $("#btnCreate").remove();
                }

                if($("#variant_medium_show").length != 1)
                {
                    $(".view_btn").remove();
                }
                if($("#variant_medium_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#variant_medium_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#variant_medium_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Variant Media Listing</h6>');
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('variant media')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
            } else if (result.dismiss === Swal.DismissReason.cancel) {
              safeRecord('variant media');
            }
        });
    });
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('variant media')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected variant media');
            }
        });
    });

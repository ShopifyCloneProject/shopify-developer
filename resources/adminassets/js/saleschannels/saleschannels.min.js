$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        return (
                             ...
                             '<a href="'+displayUrl+ '/' + full.id+'" class="btn-view" data-id="'+full.id+'" >' +
                            ...
                            '</a>' +
                            '<a href="'+ getListingUrl + '/' + full.id + '/edit' + '" class="item-edit btn-edit" data-id="'+full.id+'" >' +
            ...
            initComplete: function(){
                ...
                if($("#sales_channel_edit").length != 1)
                {
                    $(".btn-edit").remove();
                }
                if($("#sales_channel_show").length != 1)
                {
                    $(".btn-view").remove();
                }
                if($("#sales_channel_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#sales_channel_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Sales Channels Listing</h6>');
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('sales channel')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
            } else if (result.dismiss === Swal.DismissReason.cancel) {
              safeRecord('sales channel');
            }
        });
    });
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('sales channel')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected sales channel');
            }
        });
    });

$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            columns: [{
                ...
                {
                    data: 'product_or_collection',
                    name: 'product_or_collection'
                },
                {
                    data: 'status',
                    name: 'status'
            ...
            initComplete: function(){
                ...
                if($("#discounts_create").length != 1)
                {
                    $("#btnCreate").remove();
                }
                if($("#discounts_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#discounts_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#discounts_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Discounts Listing</h6>');
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('discounts')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
            } else if (result.dismiss === Swal.DismissReason.cancel) {
              safeRecord('discounts');
            }
        });
    });
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('discounts')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected discounts');
            }
        });
    });

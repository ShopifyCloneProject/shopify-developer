$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            initComplete: function(){
                ...
                if($("#order_financial_status_create").length != 1)
                {
                    $("#btnCreate").remove();
                }

                if($("#order_financial_status_show").length != 1)
                {
                    $(".view_btn").remove();
                }
                if($("#order_financial_status_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#order_financial_status_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#order_financial_status_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Order Financial Status Listing</h6>');
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('order financial status')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
            } else if (result.dismiss === Swal.DismissReason.cancel) {
              safeRecord('order financial status');
            }
        });
    });
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('order financial status')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected order financial status');
            }
        });
    });

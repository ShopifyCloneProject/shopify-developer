$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            columns: [{
                ...
                {
                    data: 'order_detail_receipt_number',
                    name: 'order_detail_receipt_number'
                },
                {
            ...
            initComplete: function(){
                ...
                if($("#order_product_variant_create").length != 1)
                {
                    $("#btnCreate").remove();
                }

                if($("#order_product_variant_show").length != 1)
                {
                    $(".view_btn").remove();
                }
                if($("#order_product_variant_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#order_product_variant_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#order_product_variant_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Order Product Variant Listing</h6>');
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('order product varivant')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
            } else if (result.dismiss === Swal.DismissReason.cancel) {
              safeRecord('order product varivant');
            }
        });
    });
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('order product varivant')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected order product varivant');
            }
        });
    });

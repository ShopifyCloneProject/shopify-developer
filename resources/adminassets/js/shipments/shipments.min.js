$(document).ready(function() {
    ...
    if (dt_filter_table.length) {
        ...
        var dt_filter_table = dt_filter_table.DataTable({
            ...
            initComplete: function(){
                ...
                if($("#shipment_status_create").length != 1)
                {
                    $("#btnCreate").remove();
                }

                if($("#shipment_status_edit").length != 1)
                {
                    $(".edit_btn").remove();
                }
                if($("#shipment_status_delete").length != 1)
                {
                    $(".delete_btn").remove();
                }
                if($("#shipment_status_export").length != 1)
                {
                    $("#exportbutton").remove();
                }
        ...
        $('div.head-label').html('<h6 class="mb-0">Shipment Status Listing</h6>');
    ...
    $('.data-submit').on('click', function(e) {
        ...
        e.preventDefault();
        var formData = {
            description: jQuery('#addDescription').val(),
            status_code: jQuery('#addStatusCode').val(),
            status: jQuery('#addStatus').val(),
            shipping_method_id: jQuery('#shipping_method_id').val()
        };
        var type = "POST";
    ...
    $('.dt-column-search tbody').on('click', '.delete-record', function(e) {
      ...
      Swal.fire(deleteConfirmBox('shipment')).then(function (result) {
            if (result.value) {
              deleteRecord(id);
    ...
    $(document).on('click', '.delete-selected', function(){
        Swal.fire(deleteConfirmBox('shipment')).then(function (result) {
            ...
            if (result.value) {
              ...
              safeRecord('all selected shipment');
            }
        });
    });
    ...
    $(document).on('click', '.item-edit', function(){
        ...
        let data = dt_filter_table.row(rowindex).data();

        $('#shipmentStatusId').val(data.id);
        $('#editDescription').val(data.description);
        $('#editStatusCode').val(data.status_code);
        let status = data.status == 'Active' ? 1 : 0;
        $('#editStatus').val(status);
        $('#shippingMethodId').val(data.shipping_method_id);
        $('#default').modal('show');
    });
    ...
    $(document).on('click', '.data-edit', function(){
        openLoader();
        let id = $('#shipmentStatusId').val(),
        description = $('#editDescription').val(),
        status_code = $('#editStatusCode').val(),
        status = $('#editStatus').val(),
        shipping_method_id = $('#shippingMethodId').val();
        ...
        $.ajax({
            ...
            success: function(data) {
                closeLoader();
                let res = data.response;
                if(res.status_code == 3068){
                    successModal(res.message, 'Edited!');
                    $('.modal').modal('hide');
                    dt_filter_table.ajax.reload();
                } else {
                     errorModal('Something went wrong!');
